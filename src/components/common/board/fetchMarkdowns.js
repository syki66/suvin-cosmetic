import { rawMarkdownUrlArrayBroker } from "./rawMarkdownUrlArray"

const fetchMarkdowns = async (path, isMain) => {
    ...
    function sliceFrontMatter (text, index) {
        ...
        index = rawMarkdownUrlArrayBroker(path, isMain).length - index; // 인덱스 순서 역순으로 변환
    
        return { index, date, title, writer, disqus, thumbnail, mainText }
    }
    
    try {
        return Promise.all(rawMarkdownUrlArrayBroker(path, isMain).map((file, i) => fetch(file).then(res => (res.text()).then(result => (sliceFrontMatter(result, i))))));
    }
    catch (err) {
